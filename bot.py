...
BOT_TOKEN = os.environ.get('BOT_TOKEN', "8769116926:AAEibjee9f8KNliD68pznm-5lLziHCUtxcs")
ADMIN_IDS = [int(id) for id in os.environ.get('ADMIN_IDS', '8135803663').split(',')]
GROUP_CHAT_ID = int(os.environ.get('GROUP_CHAT_ID', '-1003698229252'))
SUPPORT_CHAT_ID = int(os.environ.get('SUPPORT_CHAT_ID', '-1003698229252'))
CHANNEL_ID = os.environ.get('CHANNEL_ID', '@proverkabotkp')
DATABASE_URL = os.environ.get('DATABASE_URL')  # Bothost предоставляет это автоматически!
...
logger.info(f"DATABASE_URL: {'✅ Найден' if DATABASE_URL else '❌ НЕ НАЙДЕН!'}")
logger.info("=" * 50)
...
def get_db_connection():
    ...
    if not DATABASE_URL:
        logger.error("❌ DATABASE_URL не настроен! Используйте переменные окружения Bothost")
        return None
...
def init_db():
    ...
    try:
        ...
        # Таблица статистики объявлений по дням
        ...
        # Добавляем настройки бота если их нет
...
def find_user_by_username_or_id(search_term):
    """Поиск пользователя по юзернейму или ID"""
    try:
        conn = get_db_connection()
        if not conn:
            return None
        ...
        # Пробуем как юзернейм (убираем @ если есть)
...
def get_daily_stats():
    """Получает количество опубликованных постов за сегодня"""
    try:
        today = date.today().isoformat()
        conn = get_db_connection()
        if not conn:
...
def get_total_ads_sent():
    """Получает общее количество отправленных объявлений"""
    try:
        conn = get_db_connection()
        if not conn:
            return 0
...
def get_total_ads_published():
    """Получает общее количество опубликованных объявлений"""
    try:
        conn = get_db_connection()
        if not conn:
            return 0
...
async def run_bot():
    ...
    # Запускаем в режиме POLLING (надежнее для Bothost)
